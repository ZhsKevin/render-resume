import "./modulepreload-polyfill-3TwIyLEr.js";
...
async function bootstrap() {
	...
	const container = renderLayout(markdownPath, markdown.render(markdownSource), createRawSourceUrl(markdownSource));
...
function getMarkdownPath() {
	...
	if (!file) throw new Error("缺少 file 参数，例如：mdviewer.html?file=assets/markdown/project-guide.md");
	const normalizedPath = file.replace(/^\.?\//, "");
	if (!normalizedPath.startsWith("assets/markdown/")) throw new Error("当前查看器仅允许读取 assets/markdown/ 目录下的 Markdown 文件。");
	if (!normalizedPath.endsWith(".md")) throw new Error("当前查看器仅支持渲染 .md 文件。");
	return normalizedPath;
}
async function fetchMarkdown(path) {
	const response = await fetch(path, { headers: { Accept: "text/markdown, text/plain;q=0.9, */*;q=0.8" } });
	if (!response.ok) throw new Error(`文档加载失败：${response.status} ${response.statusText}`);
...
function createIcon(type) {
	return `<span class="md-shell__nav-icon">${{
		back: `
...
function rewriteDocumentLinks(container, markdownPath) {
	...
	container.querySelectorAll("[src], [href]").forEach((node) => {
		if (node.hasAttribute("src")) {
			const nextSrc = rewriteAssetUrl(node.getAttribute("src"), baseUrl);
			if (nextSrc) node.setAttribute("src", nextSrc);
		}
		if (node.hasAttribute("href")) {
			const nextHref = rewriteHref(node.getAttribute("href"), baseUrl);
			if (nextHref) {
...
function syncDocumentTitle(container, markdownPath) {
	const text = container.querySelector("h1, h2, h3")?.textContent?.trim();
...
function createRawSourceUrl(markdownSource) {
	const rawBlob = new Blob([markdownSource], { type: "text/plain;charset=utf-8" });
